...
tpot_data = pd.read_csv('C:/Users/WS01/Desktop/Github/Original Excels/Group C/CO2 - 1 BAR - 393 K - C.csv', sep=',', dtype=np.float64)
...
# Average CV score on the training set was: -0.0009000915578901659
exported_pipeline = make_pipeline(
    make_union(
        StackingEstimator(estimator=RandomForestRegressor(bootstrap=True, max_features=0.3, min_samples_leaf=1, min_samples_split=11, n_estimators=100)),
        FunctionTransformer(copy)
    ),
    RandomForestRegressor(bootstrap=True, max_features=0.5, min_samples_leaf=6, min_samples_split=11, n_estimators=100)
)
